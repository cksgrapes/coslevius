...
class MW_WP_Form_Form {
	...
	public function radio( $name, $children = array(), $options = array() ) {
		...
		foreach ( $children as $key => $_value ) {
			...
			$_ret .= sprintf(
				...
				// $attributes,
				// esc_html( $_value )
				'<input type="radio" class="form-parts__radio" name="%s" value="%s"%s %s><label%s class="form-parts__radio__label">%s</label>',
				esc_attr( $name ),
				esc_attr( $key ),
				checked( $key, $options['value'], false ),
				$attributes,
	...
	public function checkbox( $name, $children = array(), $options = array(), $separator = ',' ) {
		...
		foreach ( $children as $key => $_value ) {
			...
			$_ret .= sprintf(
				...
				// esc_html( $_value )
				'<input class="form__attention__check" type="checkbox" name="%s" value="%s"%s %s><label%s class="form__attention__check--label">%s</label>',
				esc_attr( $name . '[data][]' ),
				esc_attr( $key ),
				$attributes,
				checked( ( is_array( $value ) && in_array( $key, $value ) ), true, false ),
	...
	public function submit( $name, $value, $options = array() ) {
		...
		return sprintf(
			...
			// esc_attr( $value ),
			// $attributes
			'<button class="action-btn__item" type="submit" name="%s" value="%s"%s><span>%s</span></button>',
			esc_attr( $name ),
			esc_attr( $value ),
			$attributes,
			esc_attr( $value )
